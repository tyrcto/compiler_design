...
vector<string> IDValue::getArrTypes(){
    vector<string> types;
    for(auto&a:arr){
        types.push_back(a.getTypeStr());
    }
    return types;
}
...
bool SymbolTableList::isGlobal(string id){
    for(int i=top;i>=0;i--){
        auto temp = list[i].lookup(id);
        if(temp && temp->flag != CONST_FLAG && i == 1)return true; 
    }
    return false;
}
